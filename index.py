...
def raiz_cuadrada(a: float):
    return 'Error: raíz negativa' if a < 0 else a ** 0.5
def modulo(a: float, b: float) -> float: return a % b
def inverso(a: float):
    return 'Error: sin inverso' if a == 0 else 1 / a
...
def copiar_resultado():
    ...
    if resultado and not resultado.startswith("Error"):
        ...
        messagebox.showwarning("Atención", "No hay resultado válido para copiar.")
...
def calcular(op: str):
    try:
        ...
        num2 = float(entry2.get()) if entry2.get() and op not in ['Raiz', 'Inverso', 'Absoluto'] else None
...
for i, op in enumerate(OPERACIONES):
    row = 2 + i // 3
    col = i % 3
    tk.Button(root, text=op, width=12, command=lambda o=op: [actualizar_entrada(o), calcular(o)]).grid(row=row, column=col, padx=5, pady=5)

# Botones extra
tk.Button(root, text="Limpiar", command=limpiar).grid(row=6, column=0, pady=10)
tk.Button(root, text="Copiar Resultado", command=copiar_resultado).grid(row=6, column=1, pady=10)
tk.Button(root, text="Borrar Historial", command=borrar_historial).grid(row=6, column=2, pady=10)

# Resultado
resultado_label = tk.Label(root, text="Resultado:", font=("Arial", 12))
...
historial = tk.Listbox(frame_historial, height=8, width=60, yscrollcommand=scrollbar.set)
